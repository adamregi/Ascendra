...
@riverpod
Future<ExecutiveOverviewModel> executiveOverview(ExecutiveOverviewRef ref) {
  ref.cacheFor(const Duration(minutes: 5));
  return ref.watch(dashboardRepositoryProvider).getExecutiveOverview();
}

@riverpod
Future<LeadershipPipelineModel> leadershipPipeline(LeadershipPipelineRef ref) {
  ref.cacheFor(const Duration(minutes: 5));
  return ref.watch(dashboardRepositoryProvider).getLeadershipPipeline();
}

@riverpod
Future<AlertPreviewModel> alertPreview(AlertPreviewRef ref) {
  ref.cacheFor(const Duration(minutes: 5));
  return ref.watch(dashboardRepositoryProvider).getAlerts();
}

@riverpod
Future<RecommendationPreviewModel> recommendationPreview(RecommendationPreviewRef ref) {
  ref.cacheFor(const Duration(minutes: 5));

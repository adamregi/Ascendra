...
import '../models/task_proof_model.dart';
...
class TaskRepositoryImpl extends BaseRepository implements TaskRepository {
  ...
  Future<Task> createTask({
    ...
    try {
      final response = await _client.rpc('create_task_atomic', params: {
  ...
  Future<void> assignMembers({
    ...
    try {
      await _client.rpc('assign_task_members', params: {
        'p_task_id': taskId,
  ...
  Future<void> startAssignment({
    ...
    try {
      await _client.rpc('start_task_assignment', params: {
  ...
  Future<TaskProof> submitProof({
    ...
    try {
      final response = await _client.rpc('submit_task_proof', params: {
        'p_assignment_id': assignmentId,
        'p_proof_type': proofType,
        'p_comment': comment,
        'p_file_url': fileUrl,
      });
      return TaskProofModel.fromJson(response as Map<String, dynamic>);
    } catch (e, stack) {
      handleException(e, stack);
    }
  }
  ...
  Future<void> reviewAssignment({
    ...
    try {
      await _client.rpc('review_task_assignment', params: {
        'p_assignment_id': assignmentId,
        'p_leader_id': leaderId,
        'p_approved': approved,
        'p_comment': comment,
  ...
  Future<List<TaskAssignment>> getAssignments({required String taskId}) async {
    try {
      final taskRes = await _client.from('tasks').select('company_id').eq('id', taskId).single();
      final companyId = taskRes['company_id'] as String;
      await _client.rpc('sync_overdue_assignments', params: {
        'p_company_id': companyId,
      });
      ...
      final List<dynamic> response = await _client
          ...
          .select()
          .eq('task_id', taskId);
      return response.map((e) => TaskAssignmentModel.fromJson(e as Map<String, dynamic>)).toList();
    } catch (e, stack) {
      handleException(e, stack);
    }
  }
  ...
  @override
  Future<List<TaskAssignment>> getMemberAssignments({required String memberId}) async {
    try {
      final memberRes = await _client.from('profiles').select('company_id').eq('id', memberId).single();
